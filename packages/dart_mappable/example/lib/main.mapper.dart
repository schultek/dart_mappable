...
class PersonMapperElement extends MapperElementBase<Person> {
  ...
  Person fromMap(Map<String, dynamic> map) => Person(container.$get(map, 'name'), age: container.$getOpt(map, 'age') ?? 18, car: container.$getOpt(map, 'car'));

  @override Function get encoder => encode;
  dynamic encode(Person v) => toMap(v);
  Map<String, dynamic> toMap(Person p) => {'name': container.$enc(p.name, 'name'), 'age': container.$enc(p.age, 'age'), 'car': container.$enc(p.car, 'car')};

  @override String stringify(Person self) => 'Person(name: ${container.asString(self.name)}, age: ${container.asString(self.age)}, car: ${container.asString(self.car)})';
  @override int hash(Person self) => container.hash(self.name) ^ container.hash(self.age) ^ container.hash(self.car);
  @override bool equals(Person self, Person other) => container.isEqual(self.name, other.name) && container.isEqual(self.age, other.age) && container.isEqual(self.car, other.car);
}
...
class CarMapperElement extends MapperElementBase<Car> {
  ...
  Car fromMap(Map<String, dynamic> map) => Car(container.$get(map, 'driven_km'), container.$get(map, 'brand'));

  @override Function get encoder => encode;
  dynamic encode(Car v) => toMap(v);
  Map<String, dynamic> toMap(Car c) => {'driven_km': container.$enc(c.drivenKm, 'drivenKm'), 'brand': container.$enc(c.brand, 'brand')};

  @override String stringify(Car self) => 'Car(miles: ${container.asString(self.miles)}, brand: ${container.asString(self.brand)})';
  @override int hash(Car self) => container.hash(self.miles) ^ container.hash(self.brand);
  @override bool equals(Car self, Car other) => container.isEqual(self.miles, other.miles) && container.isEqual(self.brand, other.brand);
}
...
class BoxMapperElement extends MapperElementBase<Box> {
  ...
  @override Function get decoder => decode;
  Box<T> decode<T>(dynamic v) => checkedType(v, (Map<String, dynamic> map) => fromMap<T>(map));
  Box<T> fromMap<T>(Map<String, dynamic> map) => Box(container.$get(map, 'size'), content: container.$get(map, 'content'));

  @override Function get encoder => encode;
  dynamic encode<T>(Box<T> v) => toMap<T>(v);
  Map<String, dynamic> toMap<T>(Box<T> b) => {'size': container.$enc(b.size, 'size'), 'content': container.$enc(b.content, 'content'), ...container.$type<Box<T>>(b)};

  @override String stringify(Box self) => 'Box(size: ${container.asString(self.size)}, content: ${container.asString(self.content)})';
  @override int hash(Box self) => container.hash(self.size) ^ container.hash(self.content);
  @override bool equals(Box self, Box other) => container.isEqual(self.size, other.size) && container.isEqual(self.content, other.content);
}
