...
part of 'basic_decoding_test.dart';
...
class AMapperElement extends MapperElementBase<A> {
  ...
  A fromMap(Map<String, dynamic> map) => A(container.$get(map, 'a'), b: container.$getOpt(map, 'b') ?? 0, c: container.$getOpt(map, 'c'), d: container.$get(map, 'd'));

  @override Function get encoder => encode;
  dynamic encode(A v) => toMap(v);
  Map<String, dynamic> toMap(A a) => {'a': container.$enc(a.a, 'a'), 'b': container.$enc(a.b, 'b'), 'c': container.$enc(a.c, 'c'), 'd': container.$enc(a.d, 'd')};

  @override String stringify(A self) => 'A(a: ${container.asString(self.a)}, b: ${container.asString(self.b)}, c: ${container.asString(self.c)}, d: ${container.asString(self.d)})';
  @override int hash(A self) => container.hash(self.a) ^ container.hash(self.b) ^ container.hash(self.c) ^ container.hash(self.d);
  @override bool equals(A self, A other) => container.isEqual(self.a, other.a) && container.isEqual(self.b, other.b) && container.isEqual(self.c, other.c) && container.isEqual(self.d, other.d);
}
...
class _ACopyWithImpl<$R, $Out extends A> extends CopyWithBase<$R, A, $Out> implements ACopyWith<$R, A, $Out> {
  ...
  @override $R call({String? a, int? b, Object? c = $none, bool? d}) => $then(A(a ?? $value.a, b: b ?? $value.b, c: or(c, $value.c), d: d ?? $value.d));
}
