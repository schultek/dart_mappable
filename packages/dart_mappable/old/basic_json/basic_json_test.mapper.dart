...
class CarMapperElement extends MapperElementBase<Car> {
  ...
  Car fromMap(Map<String, dynamic> map) => Car(container.$get(map, 'drivenKm'), container.$get(map, 'brand'));

  @override Function get encoder => encode;
  dynamic encode(Car v) => toMap(v);
  Map<String, dynamic> toMap(Car c) => {'drivenKm': container.$enc(c.drivenKm, 'drivenKm'), 'brand': container.$enc(c.brand, 'brand')};
