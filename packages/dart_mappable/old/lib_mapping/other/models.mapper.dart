...
class PersonMapperElement extends MapperElementBase<Person> {
  ...
  Person fromMap(Map<String, dynamic> map) => Person(container.$get(map, 'firstName'));

  @override Function get encoder => encode;
  dynamic encode(Person v) => toMap(v);
  Map<String, dynamic> toMap(Person p) => {'firstName': container.$enc(p.firstName, 'firstName')};

  @override String stringify(Person self) => 'Person(firstName: ${container.asString(self.firstName)})';
  @override int hash(Person self) => container.hash(self.firstName);
  @override bool equals(Person self, Person other) => container.isEqual(self.firstName, other.firstName);
}
